"""Re-run the classification, which 19.0.1.20.0 got half right.

That pass mapped allowances by a single ``ALW`` category. The KSW
chart gives HRA, DA, Meal, Medical and Travel a category each, so
every one of them was left unposted — and an unposted rule with an
amount stops the export. The corrected rule defaults to *expense*
and lists the exceptions instead.
