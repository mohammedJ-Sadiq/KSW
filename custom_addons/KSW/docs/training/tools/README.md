...
for f in setup_demo_data stage_records stage_leaves stage_more; do
...
`train.accounting`, `train.gm`, `train.payroll`, `train.admin`, all with the
...
## Coverage status — 63 / 63 captured ✅
...
Data staging is split across four idempotent shell scripts, run in order:
`setup_demo_data.py` → `stage_records.py` → `stage_leaves.py` → `stage_more.py`
→ `stage_final.py`. The payslip/batch shots reuse **real existing records**
