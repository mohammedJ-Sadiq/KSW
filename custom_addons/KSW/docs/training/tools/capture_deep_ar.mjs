...
const SHOTDIR = join(__dirname, '..', 'screenshots-ar');
...
async function detectError(page) {
  return await page.evaluate(() => {
    ...
    document.querySelectorAll('.o_form_view .alert-danger, .o_form_view .alert-warning').forEach(a => {
      ...
      // the skipped-employees panel is intended, documented batch content (EN + AR)
      if (/automatically skipped during payslip generation/i.test(t)) return;
      if (/تم تخطي الموظفين|تلقائياً أثناء إنشاء/i.test(t)) return;
...
const SHOTS = [
  ...
  { tag: 'gm/return-02', persona: 'gm', run: async (page) => {
      await openRecord(page, LEAVES, IDS.leave_pending_gm_initial);
      await page.waitForTimeout(600);
      // Arabic UI: button label is "إعادة إلى المعتمد" (↩ prefix optional)
      await page.getByRole('button', { name: /إعادة إلى المعتمد/i }).first().click();
  ...
  { tag: 'payroll/export-02', persona: 'payroll', run: async (page) => {
      await openRecord(page, 'om_hr_payroll.action_hr_payslip_run_tree', IDS.payslip_batch);
      await page.waitForTimeout(500);
      // Arabic UI: "تصدير ملف التحويل البنكي" — fallback to any button containing "تصدير"
      const btn = page.getByRole('button', { name: /تصدير ملف التحويل البنكي|تصدير/i }).first();
      await btn.click();
      await page.waitForTimeout(2500);
  }},
  ...
  { tag: 'employee/attendance-01', persona: 'employee', run: async (page) => {
      // Arabic UI: app name is "الحضور" — use URL-based nav as reliable fallback
      await page.goto(`${BASE}/odoo/attendances`, { waitUntil: 'domcontentloaded' });
      await page.waitForSelector('.o_list_view, .o_kanban_view', { timeout: 15000 });
      await page.waitForTimeout(1200);
  }},
